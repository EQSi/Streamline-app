...
const CompanyDetailsPage: React.FC = () => {
    ...
    const handleContactSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (isAddingDivisionContact && company && company.hasDivisions && selectedDivision && selectedDivision.id) {
                contactResponse = await axiosInstance.post(
                    `/companies/${companyId}/divisions/${selectedDivision.id}/contacts`,
                    contactData,
                    config
                );
                // Add contact to division contacts
                setSelectedDivision({
                    ...selectedDivision,
                    contacts: [...(selectedDivision.contacts || []), contactResponse.data],
                });
                // Also add contact to global company contacts
                setContacts((prev) => [...prev, contactResponse.data]);
            } else if (isAddingContact && company) {
                contactResponse = await axiosInstance.post(
        ...
        const renderContent = () => {
            ...
            {company.hasDivisions && company.divisions && company.divisions.length > 0 &&
                ...
                <div key={division.id} className="mt-6">
                    ...
                    <div className="flex flex-row gap-4">
                        ...
                        {isEditingLocation && (
                            <form onSubmit={handleLocationSubmit} className="p-4 mb-4 border rounded space-y-2">
                        ...
                        {selectedDivision?.id === division.id && isAddingDivisionContact && (
            ...
            {/* Global Section for Companies Without Divisions */}
